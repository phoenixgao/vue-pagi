  ...
  computed: {
    ...
    pages: function () {
      ...
      if (this.pagination.current_page >= this.pagination.total_pages - this.config.noEllipsisNum + 1) {
        for (let i = Math.max(cursor + 1, Math.min(this.pagination.total_pages - this.config.noEllipsisNum + 1, this.pagination.current_page - this.config.offset)); i <= this.pagination.total_pages; i++) {
          pages.push({
            text: i,
            type: 'link'
          })
